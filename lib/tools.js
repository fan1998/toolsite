export const SITE = {
  ...
  url: process.env.NEXT_PUBLIC_SITE_URL || "https://www.fanjian.org",
};
...
export const tools = [
 ...
 {
 slug: "id-card-validate",
 category: "生活实用",
 badge: "HOT",
 title: "身份证号解析校验工具",
 description:
   "在线身份证号码校验解析工具，输入18位身份证号立即校验真伪并解析出省份、出生日期、性别、周岁。纯本地处理，号码不上传。",
 keywords: ["身份证校验", "身份证号解析", "身份证归属地", "身份证校验位"],
 intro:
   "输入18位身份证号，自动校验格式、地区码、出生日期和末位校验码是否正确，并解析出省份、生日、性别和周岁。全部计算在浏览器本地完成，号码不会上传到任何服务器。",
 faq: [
   {
     q: "身份证最后一位X是什么意思？",
     a: "第18位是按前17位数字通过 ISO 7064 MOD 11-2 算法算出的校验码，取值可能是 0-9 或 X（罗马数字10）。X 不是字母含义，纯粹是校验运算的结果。",
   },
   {
     q: "校验通过就一定是真实存在的证件吗？",
     a: "不是。校验只能证明号码本身符合国家标准格式，不能验证它是否真的被公安机关签发。本工具不会连接任何数据库查询。",
   },
   {
     q: "输入的号码会被上传吗？",
     a: "不会。校验、解析全部在你的浏览器里完成，页面不发送任何网络请求，可以放心用于测试数据排查。",
   },
 ],
 component: "IdCardValidate",
 },
 {
 ...
 title: "百分比计算器",
 description:
   "免费在线百分比计算器：求A是B的百分之几、求A的B%是多少、计算增长减少百分比，三种比例题一页搞定，实时出结果。",
 keywords: ["百分比计算", "增长率计算", "占比计算", "百分数计算器"],
 intro:
   "一页解决三种最常见的百分比问题：① A 占 B 的百分比；② A 的 B% 等于多少；③ A 比 B 增长或减少了百分之几。边输入边出结果，无需点击按钮。",
 faq: [
   {
     q: "增长百分比怎么算的？",
     a: "公式为 (现值 − 原值) ÷ |原值| × 100%。结果为正表示增加，为负表示减少。基数取绝对值，所以原值为负数时方向依然正确。",
   },
   {
     q: "为什么结果有很多位小数？",
     a: "工具保留最多4位小数以保证精度。日常使用可自行四舍五入，例如 33.3333% 通常口头说成 33.3%。",
   },
 ],
 ...
 title: "工作日计算器",
 description:
   "在线工作日/自然日计算器，自动扣除周末和法定节假日统计两个日期之间的工作日天数，支持自定义调休补班日期。",
 keywords: ["工作日计算", "天数计算", "工作日查询", "法公休日扣除"],
 intro:
   "选择起止日期，一键统计区间内的总天数和工作日天数。默认扣除周六日及内置的2026年法定节假日，调休上班日会自动计入；也可以手动补充任意年份的假期安排。",
 faq: [
   {
     q: "节假日数据覆盖哪些年份？",
     a: "内置了国务院公布的2026年全年放假调休安排（含少量2027年）。其他年份可在「自定义」中用 - 日期 / + 日期 的方式补充放假和补班日。",
   },
   {
     q: "适合用来估算项目工期吗？",
     a: "适合做粗略排期参考：填写开工和截止日期即可得到需投入的工作日数量。未考虑团队自身的年假、事假等特殊请假情况。",
   },
 ],
 component: "WorkdayCalc",
 },
 {
 slug: "tax-calculator",
 category: "生活实用",
 ...
 title: "个人所得税计算器",
 description:
   "在线个税计算器，按2019年起实施的累计预扣法估算每月应缴个税和到手工资，支持五险一金比例与专项附加扣除设置。",
 keywords: ["个税计算", "个人所得税", "工资计算", "到手工资", "累计预扣"],
 intro:
   "输入税前月薪、社保公积金基数和个人承担比例，以及每月专项附加扣除金额，按累计预扣预缴法逐月估算个税与到手工资，给出全年汇总和逐月明细表。",
 faq: [
   {
     q: "为什么每个月扣的税不一样多？",
     a: "现行个税采用累计预扣法：年初累计收入低适用低税率，随累计收入增加跨入更高税率档，月度扣税会逐渐增多——这是正常现象，全年总税额不变。",
   },
   {
     q: "和实际工资条有出入怎么办？",
     a: "本工具按「社保基数=月薪、不考虑缴费上下限」简化估算，且未包含年终奖单独计税等情形。实际以单位申报和当地社保政策为准，误差通常在几十元内。",
   },
   {
     q: "专项附加扣除是什么？",
     a: "包括子女教育、房贷利息、住房租金、赡养老人、继续教育、大病医疗和婴幼儿照护等项目，符合条件可在个税APP申报，直接减少应纳税所得额。",
   },
 ],
 ...
 title: "URL解析工具",
 description:
   "在线URL网址解析工具，拆解协议、域名、端口、路径、查询参数和锚点，逐项展示并列举每个query参数的键值。",
 keywords: ["url解析", "url拆解", "query参数", "网址分析"],
 intro:
   "粘贴任意网址，立即拆解出协议、主机名、端口、路径、查询字符串和哈希锚点，并把 query 参数逐个列出键值对，调试接口回调地址、埋点链接时非常顺手。",
 faq: [
   {
     q: "没有协议头的链接能解析吗？",
     a: "需要带 http:// 或 https:// 协议头。浏览器标准要求 URL 必须有协议才合法，纯域名请自行补上前缀再粘贴。",
   },
   {
     q: "URL里的中文和%编码怎么看原文？",
     a: "工具在显示参数值时已经过自动解码处理（基于 URLSearchParams），例如 %E4%B8%AD 会以「中」呈现。",
   },
 ],
 component: "UrlParser",
 },
 ...
 title: "Cron表达式解析工具",
 description:
   "在线Cron表达式解析器，把5字段cron翻译成中文说明，自动预测接下来5次执行时间，附常用表达式示例速查。",
 keywords: ["cron表达式", "定时任务", "crontab解析", "cron下次执行时间"],
 intro:
   "输入标准的5字段 Cron 表达式（分 时 日 月 周），工具会用中文逐字段解释触发规则，并按本机时区预测接下来的5次执行时间，写 n8n / crontab 定时任务前先验证一遍最稳妥。",
 faq: [
   {
     q: "支持哪些语法？",
     a: "支持 * 、? 、逗号列表、连字符范围、/ 步进以及 SUN-SAT、JAN-DEC 英文缩写。不支持 L W # 等 Quartz 扩展符号。",
   },
   {
     q: "同时填了「日」和「周」时怎么判定？",
     a: "遵循标准 cron 语义：两者都受限时为「或」关系——日匹配或周匹配任一满足即触发。这也是最常被误解的一条规则。",
   },
   {
     q: "执行时间是按什么时区算的？",
     a: "按你打开页面这台设备的本地时区计算。服务器 crontab 通常遵循服务器时区，n8n 默认可配置，跨时区使用时注意换算。",
   },
 ],
 ...
 title: "CSV与JSON互转工具",
 description:
   "免费在线CSV转JSON、JSON转CSV工具，支持自定义分隔符、首行表头开关，标准引号转义处理，纯浏览器本地转换。",
 keywords: ["csv转json", "json转csv", "表格转换", "excel转json"],
 intro:
   "左侧贴 CSV 右侧得 JSON，反向亦可。支持逗号/Tab/分号/竖线分隔符切换、首行是否作为表头、引号包裹含换行字段的完整转义规则，数字列自动转为数值类型。",
 faq: [
   {
     q: "可以直接把 Excel 内容粘进来吗？",
     a: "可以。Excel 复制出来的单元格默认以 Tab 分隔，把分隔符切到「制表符 Tab」再转换即可，无需另存为 CSV 文件。",
   },
   {
     q: "字段里含逗号或换行怎么办？",
     a: "工具实现了 RFC 4180 标准：用双引号包裹的字段内部可以有逗号、换行，连续两个双引号表示一个字面双引号，都能被正确还原。",
   },
   {
     q: "JSON→CSV 遇到嵌套对象怎么办？",
     a: "嵌套的数组或对象会序列化成 JSON 字符串放进单元格，保证信息不丢失。建议先用其他工具拍平再转。",
   },
 ],
 component: "CsvJson",
 },
 ...
 title: "HTML在线编辑运行器",
 description:
   "免费在线HTML编辑运行工具，左侧写HTML/CSS/JS右侧沙箱实时预览，支持在新窗口打开成品，代码本地处理不上传。",
 keywords: ["html在线编辑", "html预览", "在线运行html", "html测试"],
 intro:
   "左边编写 HTML + 内联 CSS 和 JavaScript，右边 iframe 沙箱实时渲染，停止输入半秒后自动刷新；还可以一键在新窗口打开成品页面。适合快速试验布局和脚本片段。",
 faq: [
   {
     q: "脚本能执行吗？安全吗？",
     a: "能。预览放在 sandbox iframe 中只开放脚本权限，JS 只在隔离环境里运行，无法访问本站 Cookie 或跳转劫持页面；你的代码也不会被上传。",
   },
   {
     q: "能加载外部图片或 CDN 资源吗？",
     a: "可以引用外部 https 图片和样式。若资源站点拒绝跨域，则需要在对方的响应头允许情况下才能加载成功。",
   },
   {
     q: "代码会自动保存吗？",
     a: "当前版本不持久保存，刷新后恢复示例内容。要留存代码请复制到本地编辑器保存。",
   },
 ],
 ...
 title: "CSS格式化压缩工具",
 description:
   "在线CSS美化与压缩工具：粘贴样式表一键格式化为缩进美观的多行结构，或压缩成单行减小体积，显示压缩率。",
 keywords: ["css格式化", "css压缩", "css美化", "css minify"],
 intro:
   "粘贴 CSS 后点「美化」得到两空格缩进的整洁排版，点「压缩」去除注释和多余空白输出单行并显示体积减小百分比，均可一键复制结果。",
 faq: [
   {
     q: "会改变样式的效果吗？",
     a: "不会。美化和压缩只是重排空白和删除注释，所有选择器、属性和值保持原样，属于等价变换。",
   },
   {
     q: "支持 SCSS / LESS 吗？",
     a: "只针对标准 CSS 语法做了优化。SCSS 的变量、嵌套等预处理语法不在支持范围内，编译成 CSS 后再处理即可。",
   },
   {
     q: "生产环境应该用它压缩 CSS 吗？",
     a: "应急够用，但正式构建建议走 cssnano、lightningcss 等构建链工具，它们还包含前缀处理、去重复规则等更多优化。",
   },
 ],
 ...
 title: "Emoji表情大全",
 description:
   "免费在线Emoji表情复制工具，按笑脸手势动物食物旅行物品分组陈列200+常用表情，点击即复制，无需安装输入法。",
 keywords: ["emoji大全", "表情符号复制", "颜文字emoji", "emoji对照表"],
 intro:
   "精选笑脸与人、手势、动物、食物、庆祝、旅行、物品等七组共两百余个高频 emoji，按分类陈列，点一下就进剪贴板，发标题、群公告、文档点缀不再到处翻找。",
 faq: [
   {
     q: "复制的 emoji 到处都能用吗？",
     a: "绝大多数场景可用，最终显示效果取决于对方平台的字体设计。部分新推出的 emoji 在旧系统上可能显示为方框。",
   },
   {
     q: "为什么一些图标像方块或问号？",
     a: "那是 Unicode 变体选择符造成的字体差异，不同设备渲染策略不同，正文环境一般无碍。",
   },
 ],
